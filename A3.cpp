...
static bool find_terminal(char p, vector<char> &v);
...
public:
        ...
        while (true)
        {
            ...
            while (isNonTerminalPresent(newNonTerminalName) || (charToStr.find(newNonTerminalName) != charToStr.end()))
            {
                if (newNonTerminalName == 'Z')
                    cout << "The current code has exceeded Z. thenkyou -- not an error, the code works." << endl;
    ...
    void direct_recursion()
    {
        ...
        if (alpha.empty())
            return; // No left recursion yipeee ;p
        ...
        do
        {
            ...
            for (Non_Terminal *temp = head; temp != nullptr; temp = temp->next_symbol)
            {
                ...
                for (const string &prod : temp->Production)
                {
                    ...
                    {
                        if (temp == targetNT)
                            continue; // Skip same non-terminal
...
// helper to pretty-print the stack from bottom→top
static string stackToString(stack<string> st)
{
    vector<string> elems;
    while (!st.empty())
    {
        string top = st.top();
        if (charToStr.find(top[0]) != charToStr.end())
        {
            elems.push_back(charToStr[top[0]]);
        }
        else
        {
            elems.push_back(top);
    ...
    // elems now top→bottom, so print in reverse
    string s = "[ ";
...
// parse one line of tokens (each token is a single char) and show each step
#include <fstream> 
string parseLine(const vector<string> &input, int lineNumber, bool print)
{
    ofstream outfile("parsing_output_line_" + to_string(lineNumber) + ".txt"); // Output file
    ...
    if (print) outfile << "--- Parsing line " << lineNumber << " ---\n";
    outfile << left << setw(35) << "INPUT" << setw(35) << "MATCH" << "STACK\n"; 
    outfile << string(90, '-') << "\n"; // separator line

    string error_string = "";
    stack<string> st;
    st.push("$");
    string startSym(1, Non_Terminal::head->name);
    st.push(startSym);

    string previous_terminal = "";
    int idx = 0;
    bool error = false;
    while (!st.empty() && idx < (int)input.size())
    {
        Non_Terminal *nt = Non_Terminal::head;
        string top = st.top();
        string lookahead = input[idx];
        string abc;
        ...
        if (abc == lookahead) // Match
        {
            st.pop();
            ++idx;
            action = "MATCH         '" + lookahead + "'";
            if (lookahead != "$")
                previous_terminal = lookahead;

        }
        else if (nt->find_terminal(top[0], terminalSSS) || top == "$")
        {
            action = "Error: unexpected '" + lookahead + "', expected '" + abc + "'";
            error = true;
            if (lookahead == "$")
                error_string = "Syntax Error: unexpected end of input, expected '" + abc + "' (missing symbols)";
            else if (previous_terminal == "")
                error_string = "Syntax Error: unexpected '" + lookahead + "' at the start of production";
            else if (nt->find_terminal(top[0], terminalSSS))
            {
                error_string = "Syntax Error: '" + lookahead + "', expected '" + abc + "' after '" + previous_terminal + "'";
                string ab = charToStr[top[0]];
                if (idx + 1 < input.size() && ab == input[idx+1])
                    error_string += " before '" + ab + "'";
                error_string += " (mismatched terminal)";
            }
            else
            {
                if (top[0] == '$')
                    error_string = "Syntax Error: expected end of string (stack is empty)";
                else
                    error_string = "Syntax Error: unexpected '" + lookahead + "', expected '" + abc + "' after '" + previous_terminal + "' (missing symbols)";
            }
            break;
        }
        else
        {
            while (nt && nt->name != top[0])
                nt = nt->next_symbol;

            if (!nt)
            {
                action = "Error: unknown non-terminal '" + top + "'";
                error = true;
                error_string = "Logical Error: unknown non-terminal '" + top + "' in the code";
                break;
            }
            ...
            if (lookahead != "$")
            {
                auto charIt = strToChar.find(lookahead);
                if (charIt == strToChar.end())
                {
                    action = "Error: unknown terminal '" + lookahead + "'";
                    error = true;
                    error_string = "Syntax Error: unknown symbol '" + lookahead + "' to CFG";
                    break;
                }
                lc = charIt->second;
            }
            ...
            auto it = nt->ParsingTable.find(lc);
            if (it == nt->ParsingTable.end())
            {
                action = "Error: no rule for " + top + " on input '" + lookahead + "'";
                error = true;
                if (lookahead == "$")
                    error_string = "Syntax Error: unexpected end of input. (mismatched productions)";
                else if (previous_terminal == "")
                    error_string = "Syntax Error: unexpected '" + lookahead + "' at start of production";
                else
                    error_string = "Syntax Error: unexpected '" + lookahead + "' after '" + previous_terminal + "' (no required productions)";
                break;
            }
            ...
            action = "Generate " + top + " → " + (prod == "^" ? "ε" : prod);
        }

        // Save to file
        outfile << left << setw(35) << lookahead << setw(35) << action << stackToString(st) << "\n";
    }

    if (!error && st.empty() && idx == (int)input.size())
    {
        outfile << "\nResult: LINE " << lineNumber << " parsed successfully.\n";
        outfile.close();
        return "parsed successfully.";
    }
    else if (!error)
    {
        outfile << "\nResult: Line " << lineNumber << " parsing failed. Stack size = " << st.size()
                << ", remaining input starting at idx " << idx << "\n";
        outfile.close();
...
// read each line from a file, tokenize on spaces, and call parseLine()

void printTokensAfterEndMarker(const vector<string> &tokens, int lineno)
{
    cout << "Line " << lineno << ": ";
...
void parseInputFile(const string &filename)
{
    ...
    while (getline(in, line))
    {
        ...
        {
            if (!tok.empty())
                input_line.push_back(tok);  // push full token, not just first character
        }
        input_line.push_back("$"); // end-marker
        printTokensAfterEndMarker(input_line, lineNo);
        
        string s = parseLine(input_line, lineNo, 1) ; 
        if (s == "parsed successfully.")
        {
            ++successCount;
        }
        else
    ...
    cout << "\n\nParsing completed.\n";
    cout << "Total lines parsed: " << lineNo << endl;
    cout << "Successful parses: " << successCount << endl;
    cout << "Errors encountered: " << errorCount << endl;
   
    for (const auto &error : errors)
    {
        cout << "Line" << error.first << ": " << error.second << endl;
    }
...
int main()
{
 ...
 // cout << "\nLL(1) Parsing Table:\n";
    ...
    closeParsingTableFile();
    // now invoke the new parser on your test file:
    parseInputFile("string.txt");
